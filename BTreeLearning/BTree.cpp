...
template <class U>
Node<U>* Node<U>::BTreeInsert(Node<U>* root, U data) {
...
template <class U>
bool Node<U>::insertNode(Node<U>* node, U data, Entry<U>*& upEntry) {
	bool taller;
	if (node == nullptr) {
		upEntry->data = data;
		...
		int entryNdx = searchNode(node, data); // data is key now
...
int Node<U>::searchNode(Node<U>* nodePtr, U target) {
	int walker;
	if (target <= nodePtr->entries[1]->data)
		walker = 0;
	else {
		walker = nodePtr->numEntries;
		while (target <= nodePtr->entries[walker]->data) {
			walker = walker - 1;
		}
	}
	return walker;
...
int Node<U>::searchNode(U target) {
	int walker;
	if (target <= entries[1]->data)
		walker = 0;
	else {
		walker = numEntries;
		while (target <= entries[walker]->data) {
			walker = walker - 1;
		}
	}
	return walker;
...
void Node<U>::printTree(Node<U>* node, Node<U>* root) {
	...
	while (count <= node->numEntries) {
		std::cout << node->entries[count]->data;
	...
	if (node == nullptr) { // didn't find the entry you want to delete
		...
		if (tempNode->numEntries == 0) {
			if (leftSibL != nullptr && leftSibL->numEntries > 1) {
				tempNode->entries[1]->data = node->entries[ptrPos]->data;
				node->entries[ptrPos]->data = leftSibL->entries[leftSibL->numEntries]->data;
				...
				tempNode->entries[1]->data = node->entries[ptrPos]->data;
				node->entries[ptrPos]->data = rightSibl->entries[1]->data;
				...
				leftSibL->numEntries++;
				leftSibL->entries[leftSibL->numEntries]->data = node->entries[1]->data;
...
void Node<U>::removeFromNonLeaf(int idx) {
	...
	if (idx == 1 && firstPtr->numEntries >= 1) {
		U pred = getPred(1);
		entries[idx]->data = pred;
		firstPtr->deleteEntry(pred, check);
	}
